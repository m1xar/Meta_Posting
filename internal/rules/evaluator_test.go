...
func TestWindowWiderThanTheObjectsLifeIsComplete(t *testing.T) {
	t.Parallel()

	// A total-spend cap carries a long window, but a campaign minutes old has
	// no earlier history to wait for. Treating that as incomplete made every
	// launch guard inert: it would wait out the whole lookback before it
	// could act, which is precisely the period the cap exists to cover.
	rule := Rule{
		Name:          "Pause once spend reaches 1.00",
		TargetLevel:   TargetCampaign,
		Action:        ActionPause,
		WindowSeconds: int64((30 * 24 * time.Hour) / time.Second),
		Conditions: Group{
			Logic:      LogicAll,
			Conditions: []Condition{{Metric: "spend", Operator: OperatorGTE, Threshold: 1}},
		},
	}
	now := time.Date(2026, 8, 21, 12, 0, 0, 0, time.UTC)
	bornAt := now.Add(-9 * time.Minute)

	result, err := Evaluate(rule, EvaluationContext{
		Now:             now,
		ActiveSince:     bornAt,
		WindowStartedAt: bornAt,
		WindowEndedAt:   now,
		Metrics:         map[string]float64{"spend": 1.25},
	})
	if err != nil {
		t.Fatalf("Evaluate() error = %v", err)
	}
	if !result.Matched {
		t.Fatalf("nine minutes is the object's whole life, so the window is complete: %+v", result)
	}
}

func TestWindowShorterThanRequestedStillBlocksAnOlderObject(t *testing.T) {
	t.Parallel()

	// The completeness check still matters: an object that predates the
	// window must not be judged on a partial slice of it.
	rule := Rule{
		Name:          "Spend in the last day",
		TargetLevel:   TargetCampaign,
		Action:        ActionPause,
	...
	result, err := Evaluate(rule, EvaluationContext{
		Now:             now,
		ActiveSince:     now.Add(-90 * 24 * time.Hour),
	...
	if result.Matched || !containsReason(result.Reasons, "window is incomplete") {
		t.Fatalf("an object older than the window must not be judged on a slice of it: %+v", result)
	}
}
