...
func (s *Service) evaluateObject(
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return s.saveSkippedEvaluation(ctx, record, object, now, next, "the requested rolling window is not complete")
	}
	if err != nil {
		return err
	}
	tolerance := ruleBaselineTolerance(s.Config.Worker.InsightsInterval)
	if !baselineWithinTolerance(targetStart, older.WindowEnd, tolerance) {
		return s.saveSkippedEvaluation(
			ctx,
			record,
			object,
