...
// UpsertMirror records the Meta-side copy of a rule.
func (r *RuleRepository) UpsertMirror(ctx context.Context, mirror *domain.RuleMirror) error {
	return r.db.WithContext(ctx).Clauses(clause.OnConflict{
		Columns: []clause.Column{{Name: "rule_id"}, {Name: "ad_account_id"}},
		DoUpdates: clause.AssignmentColumns([]string{
			"meta_rule_id", "status", "last_error", "updated_at",
		}),
	}, clause.Returning{}).Create(mirror).Error
}

// ActiveMirrors returns the live Meta-side copies of a rule.
func (r *RuleRepository) ActiveMirrors(ctx context.Context, ruleID uuid.UUID) ([]domain.RuleMirror, error) {
	var mirrors []domain.RuleMirror
	err := r.db.WithContext(ctx).
		Where("rule_id = ? AND status = 'active' AND meta_rule_id <> ''", ruleID).
		Find(&mirrors).Error
	return mirrors, err
}

func (r *RuleRepository) MarkMirrorRemoved(ctx context.Context, id uuid.UUID, at time.Time) error {
	return r.db.WithContext(ctx).Model(&domain.RuleMirror{}).
		Where("id = ?", id).
		Updates(map[string]any{"status": "removed", "updated_at": at}).Error
}
